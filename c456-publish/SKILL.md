...
### 5.0 首段即摘要（替代 `--description`）

⚠️ **已知限制**：`playbook update --description` 传参不生效，`list_summary` 始终从正文首段自动截取。因此必须把正文第一段写成独立摘要。

**规则：** 正文第一段（导语 blockquote 或首段文字）必须是能在列表页独立展示的摘要。用户在列表扫一眼就能判断「这条内容跟我有没有关系」。

```markdown
# 正文写法示例（净稿）：
> 一个做付费软件的公司，帮助中心用的是免费协作文档——这在很多人看来只是「工具选型」问题。但从客户的角度看，这是一个关于你靠不靠谱的判断题。
```

**自检：**
- [ ] 正文第一段是否能在 30-100 字内讲清「发生了什么、为什么重要」？
- [ ] 第一段不以「在……的今天」「本文介绍了……」开篇？
- [ ] 如果没有 `--description`，正文首段截取后读起来是否完整、不突兀？
