...
| 目录 | 中文名 | 用途 | 所属父目录 |
|------|--------|------|-----------|
| `concepts/` | 概念 | 理论、方法、策略、定义 | `wiki/` |
| `entities/` | 实体 | 人、机构、组织 | `wiki/` |
| `threads/` | 线索 | 多页串联的方法论、全书脉络 | `wiki/` |
| `sources/` | 来源 | 书籍/文章/PDF 摘要 | `wiki/` |
| `products/` | 产品 | 工厂型号、规格参数 | `wiki/` |
| `substances/` | 物质 | 化学元素、化合物、分子 | `wiki/` |
| `cases/` | 案卷 | 法律案卷、判例 | `wiki/` |
| `articles/` | 文章 | 外部文章、报告 | `raw/` |
| `books/` | 书籍 | 书籍逐页提取 | `raw/` |
| `papers/` | 论文 | 学术论文 | `raw/` |
| `quotes/` | 语录 | 引述摘录 | `raw/` |
| `resources/` | 资源 | 链接合集 | `raw/` |
| `tools/` | 工具 | 工具笔记 | `raw/` |
| `work/` | 工作 | 未整理的碎片素材 | `raw/` |

> 所有目录名使用英文小写复数。新类型出现时先在此登记再创建。
